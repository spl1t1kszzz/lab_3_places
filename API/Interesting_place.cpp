...
namespace Places::API {
    ...
    asio::awaitable<std::string> Interesting_place::get_interesting_places(const json& location) {
        ...
        for (int i = 0; i < json::parse(response)["features"].size(); ++i) {
            ss << JSON_Formatter::format_print(Config::INT_PLACE_KEYS, json::parse(response)["features"][i]) +
                    JSON_Formatter::format_print(
                        Config::INT_PLACE_KEYS, json::parse(response)["features"][i]["properties"]) << std::endl;
