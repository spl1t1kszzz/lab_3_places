...
namespace Places {
    ...
    asio::awaitable<void> Finder::start(const std::string&place) {
        ...
        const std::map<std::string, bool> keys{
            {"postcode", false}, {"country", false}, {"state", false}, {"name", false}
        };
        ...
        co_spawn(co_await asio::this_coro::executor,
                 [&json, &chosen_location] { return get_weather(json[chosen_location - 1]); },
                 as_tuple(asio::detached));
    }
    ...
    asio::awaitable<void> Finder::get_weather(const json&location) {
        ...
        const std::map<std::string, bool> keys{{"temp", true}, {"feels_like", true}};
        std::cout << format_print(keys, json::parse(response)["main"]) << std::endl;
        co_return;
    }

    std::string Finder::format_print(const std::map<std::string, bool>&keys, const json&object) {
        std::stringstream ss;
        for (const auto&[key, display]: keys) {
            if (object.contains(key)) {
                ...
                if (object[key].is_string()) {
                    ss << object[key].get<std::string>() + ", ";
                }
                else if (object[key].is_number_float()) {
                    ss << std::to_string(object[key].get<float>()) + ", ";
                }
            }
        }
